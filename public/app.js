 ...
 * Modular Vanilla JavaScript Implementation with API & LocalStorage Fallback
 */
...
(function () {
  ...
  // Pre-populated default members fallback
  ...
  // Connection Check (Dual Storage Mode)
  async function checkServerConnection() {
    try {
      const res = await fetch('/api/health', { method: 'GET' });
      if (res.ok) {
        state.isOnlineServer = true;
        elements.storageBadge.innerHTML = `<span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span> Express Sync`;
  ...
  async function loadInitialData() {
    if (state.isOnlineServer) {
      try {
        ...
        state.attendance[state.currentDate] = attData;
      } catch (err) {
        console.error("Failed loading from server, reading localStorage backup", err);
  ...
  function loadLocalStorageData() {
    ...
    if (localAtt) {
      state.attendance = JSON.parse(localAtt);
    } else {
      state.attendance = {};
    }
  }
  ...
  // Helper: Generates Avatar Color & Initials
  ...
  function getAvatarBgColor(id) {
    const colors = [
      'bg-indigo-600', 'bg-emerald-600', 'bg-teal-600', 
      'bg-violet-600', 'bg-amber-600', 'bg-rose-600', 'bg-cyan-600'
    ];
    let hash = 0;
  ...
  async function toggleAttendance(memberId, targetStatus) {
    ...
    // If clicking the status that's already active, clear status (unmark)
    ...
    if (newStatus) {
      state.attendance[state.currentDate][memberId] = {
        status: newStatus,
        time: timeString,
        note: ""
      };
    ...
    if (state.isOnlineServer) {
      try {
        if (newStatus) {
          ...
          // Send reset for member if unmarking
          await fetch('/api/attendance', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        console.error("Failed posting status update to server", err);
      }
    }
    ...
    if (newStatus === 'present') {
      showToast(`Marked ${memberName} as Present`, 'success');
    } else if (newStatus === 'late') {
      showToast(`Marked ${memberName} as Late`, 'warning');
    } else if (newStatus === 'absent') {
      showToast(`Marked ${memberName} as Absent`, 'danger');
    } else {
      showToast(`Reset attendance for ${memberName}`, 'info');
    }
  }
  ...
  async function handleBulkAction(action) {
    ...
    if (action === 'mark_all') {
      state.members.forEach(member => {
        state.attendance[state.currentDate][member.id] = {
          status: 'present',
          time: timeString,
          note: ''
        };
      ...
      if (state.isOnlineServer) {
        try {
          await fetch('/api/attendance/bulk', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ date: state.currentDate, action: 'mark_all', status: 'present' })
          });
        } catch (e) {}
      }
      ...
      if (state.isOnlineServer) {
        try {
          await fetch('/api/attendance/bulk', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ date: state.currentDate, action: 'reset' })
          });
        } catch (e) {}
      }
  ...
  function setupEventListeners() {
    ...
    // Table One-Tap Button Delegation & Delete Button Delegation
    ...
    elements.addMemberModal.addEventListener('click', (e) => {
      if (e.target === elements.addMemberModal) {
        closeAddModal();
      }
    });
  }

  // Utility: Sanitize HTML string to prevent XSS
